...
module tb;
  ...
  initial begin
    ...
    // SPI readback self-test (write then write_readinto), plaintext=0x59C359C3
    cs_n=0; shift(32'h59C359C3,0); cs_n=1;
    cs_n=0; shift(32'h59C359C3,1); cs_n=1;
    $display("SPI readback rx=%08x (expect 59c359c3)", rx);
    // Encrypt
    enc_dec=0;
    start=1; pulse; start=0;
    $display("after start busy=%b (expect 1)", busy);
    repeat(7) pulse;
    $display("after 7 pulses busy=%b (expect 0)", busy);
    cs_n=0; shift(32'h59C359C3,1); cs_n=1;
