...
module tb;
  ...
  reg [31:0] DIN; reg ED; integer NC,c;
  ...
  initial begin
    ...
    if(!$value$plusargs("nc=%d",NC)) NC=3;
    sck=0;rst_n=1;cs_n=1;start=0;enc_dec=0;mosi=0;#10;
    rst_n=0;pulse;rst_n=1;pulse; enc_dec=ED;
    cs_n=0; shift(DIN); cs_n=1;
    start=1; pulse; start=0;
    for(c=0;c<NC;c=c+1) pulse;
    #1; $display("OUT=%08x busy=%b",`REG,busy);
    $finish;
  end
endmodule
